...
/**
 * Homologus tree-swapping crossover with additional statistics.
 * 
 * @author Tomasz Pawlak
 */
public class HGPStats extends HGP {
		...
		for (int q = start; q < n + start; /* no increment */) // keep on going until we're filled up
		{
			...
			for (int x = 0; x < j1.trees.length; x++) {
				if (x == t1 && res1) // we've got a tree with a kicking cross position!
				{
					...
					ReplaceResult copy = GeometricHelpers.cloneReplacing(parents[0].trees[x].child, p2, p1);
					j1.trees[x].child = copy.root;
					j1.trees[x].child.parent = j1.trees[x];
					j1.trees[x].child.argposition = 0;
					j1.evaluated = false;
					GeometricHelpers.checkGeometricity((SimpleNodeBase) p2, (SimpleNodeBase) p1,
							(SimpleNodeBase) copy.insertedNode, stat);
				} // it's changed
				else {
					j1.trees[x] = (GPTree) (parents[0].trees[x].lightClone());
					j1.trees[x].owner = j1;
			...
			if (n - (q - start) >= 2 && !tossSecondParent)
				for (int x = 0; x < j2.trees.length; x++) {
					if (x == t2 && res2) // we've got a tree with a kicking cross position!
					{
						...
						ReplaceResult copy = GeometricHelpers.cloneReplacing(parents[1].trees[x].child, p1, p2);
