...
public class GPXStats extends CrossoverPipeline {
	...
	public void setup(EvolutionState state, Parameter base) {
		...
		this.stat = (CrossoverStatistics) state.statistics;
	}
		...
		for (int q = start; q < n + start; /* no increment */) // keep on going until we're filled up
		{
			...
			for (int x = 0; x < j1.trees.length; x++) {
				if (x == t1 && res1) // we've got a tree with a kicking cross position!
				{
					...
					stat.crossoverOccurred(p1.atDepth() + 1, 1, ((SimpleNodeBase<?>) j1.trees[x].child).getSemantics()
							.isBetween(parent1Semantics, parent2Semantics));
				} // it's changed
				else {
					j1.trees[x] = (GPTree) (parents[0].trees[x].lightClone());
					j1.trees[x].owner = j1;
			...
			if (n - (q - start) >= 2 && !tossSecondParent)
				for (int x = 0; x < j2.trees.length; x++) {
					if (x == t2 && res2) // we've got a tree with a kicking cross position!
					{
						...
						stat.crossoverOccurred(p1.atDepth() + 1, 1, ((SimpleNodeBase<?>) j2.trees[x].child)
								.getSemantics().isBetween(parent1Semantics, parent2Semantics));
					} // it's changed
					else {
						j2.trees[x] = (GPTree) (parents[1].trees[x].lightClone());
						j2.trees[x].owner = j2;
