...
public class AGXStats extends AGX {

	private CrossoverStatistics stat;

	@Override
	public void setup(EvolutionState state, Parameter base) {
		super.setup(state, base);
		stat = (CrossoverStatistics) state.statistics;
	}
	...
	public int produce(int min, int max, int start, int subpopulation, Individual[] inds, EvolutionState state,
		...
		for (int q = start; q < n + start; /* no increment */) // keep on going until we're filled up
		{
			...
			ISemantics parent1Semantics = ((SimpleNodeBase) parents[0].trees[t1].child).getSemantics();
			ISemantics parent2Semantics = ((SimpleNodeBase) parents[1].trees[t2].child).getSemantics();
			ISemantics avgSemantics = parent1Semantics.getMidpointBetweenMeAnd(parent2Semantics);
			...
			for (int x = 0; x < j1.trees.length; x++) {
				if (x == t1 && res1) // we've got a tree with a kicking cross position!
				{
					...
					stat.crossoverOccurred(
							replacement1.currentNode.atDepth() + 1,
							1,
							((SimpleNodeBase) j1.trees[x].child).getSemantics().isBetween(parent1Semantics,
									parent2Semantics));
				} // it's changed
				else {
					j1.trees[x] = (GPTree) (parents[0].trees[x].lightClone());
			...
			if (n - (q - start) >= 2 && !tossSecondParent)
				for (int x = 0; x < j2.trees.length; x++) {
					if (x == t2 && res2) // we've got a tree with a kicking cross position!
					{
						...
						stat.crossoverOccurred(
								replacement2.currentNode.atDepth() + 1,
								1,
								((SimpleNodeBase) j2.trees[x].child).getSemantics().isBetween(parent1Semantics,
										parent2Semantics));
					} // it's changed
					else {
						j2.trees[x] = (GPTree) (parents[1].trees[x].lightClone());
