...
public class NonRandomizedLGX extends NonRandomizedNHX {
	...
	private static final Parameter DEFAULT_BASE = GPBreedDefaults.base().push("NonRandomizedLGX");
			...
			if (n - (q - start) >= 2 && !tossSecondParent)
				for (int x = 0; x < j2.trees.length; x++) {
					if (x == t2) // we've got a tree with a kicking cross position!
					{
						...
						if (this.random) {
							newSubtree = this.getAverageProgram(thread, p1, p2, state.random[thread], maxDepth);
						} else {
