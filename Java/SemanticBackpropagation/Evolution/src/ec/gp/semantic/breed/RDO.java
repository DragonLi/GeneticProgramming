...
/**
 * Globally Geometric Semantic Crossover. The variant, that chooses the replacement locus randomly.
 * 
 * @author Tomasz Pawlak
 * 
 */
public class RDO extends MutationPipeline {

	private static Parameter DEFAULT_BASE = GPBreedDefaults.base().push("RDO");
	...
	public int produce(int min, int max, int start, int subpopulation, Individual[] inds, EvolutionState state,
		...
		for (int q = start; q < n + start; q++) {
			...
			for (int x = 0; x < numTries; x++) {
				...
				ErrorNodePair replacement = inverter.getBestReplacementFor((SimpleNodeBase<?>) p1,
						this.maxDepth - p1.atDepth());
				if (replacement == null) {
					continue;
				} else {
					res = true;
